...
function HeroSectionCard({
  ...
  return (
    <div
      className={`shadow-drop-shadow flex flex-col gap-[10px] w-[274px] rounded-[20px] py-[25px] px-[25px] absolute ${className}`}
    >
      <div>{icon}</div>
      <div>
        <h3 className="text-[18px] font-medium">{title}</h3>
        <p className="text-[14px] mt-[5px]">{description}</p>
      </div>
    </div>
  );
}
    ...
    <div className="flex max-w-[1350px] mx-auto px-[25px] justify-between mt-[11rem] min-h-[480px]">
      ...
      <div className="flex-1">
        ...
        <div className="flex gap-[20px] mt-[30px]">
          <div className="border-none bg-[hsl(189,62%,25%)] cursor-pointer text-white rounded-[15px] px-[25px] py-[10px] hover:bg-[hsl(189,62%,30%)] transition shadow-drop-shadow hover:translate-y-[-2px]">
            Start your journey
          </div>

          <div className="border border-[hsl(189,62%,25%)] cursor-pointer rounded-[15px] px-[25px] py-[10px] hover:translate-y-[-2px] text-[hsl(189,62%,25%)] transition hover:shadow-drop-shadow">
            Take Assessment
          </div>
        </div>
      </div>
